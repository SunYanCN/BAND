...
from band.utils import SquadInputFeatures, parallel_apply
...
def parallel_squad_convert_examples_to_features(examples, tokenizer, max_length,
                                                doc_stride, max_query_length, is_training,
                                                cls_token_at_end=False,
                                                cls_token='[CLS]', sep_token='[SEP]', pad_token=0,
                                                sequence_a_segment_id=0, sequence_b_segment_id=1,
    ...
    for i, feature in enumerate(features):
        feature.example_index = i
        feature.unique_id = feature.unique_id + i
...
def squad_get_dataset_from_tf_record(tf_record_file, seq_length, batch_size=1, shuffle_buffer_size=0,
                                     is_training=False):
    if is_training:
        features = {
...
def squad_answer_type_get_dataset_from_tf_record(tf_record_file, seq_length, batch_size=1, shuffle_buffer_size=0,
                                                 is_training=False):
    if is_training:
        features = {
            "unique_ids": tf.io.FixedLenFeature([], tf.int64),
